...
const Calendar: React.FC<CalendarProps> = ({ workouts, onSelectDate, onDeleteWorkout, onRepeatWorkout }) => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    days.push(
      <div key={d} onClick={() => onSelectDate(dateStr)} className={`h-28 border border-slate-100 p-1 relative cursor-pointer hover:bg-blue-50 transition-colors ${isToday ? 'bg-brand-50 ring-1 ring-inset ring-brand-200' : 'bg-white'}`}>
        ...
        <div className="mt-1 space-y-1 overflow-y-auto h-20 scrollbar-thin scrollbar-thumb-slate-200">
            {dayWorkouts.map(w => (
                <div key={w.id} className="group relative bg-white border-l-4 border-brand-500 pl-2 pr-1 py-1 rounded-r border-y border-r border-slate-100 shadow-sm hover:shadow-md transition-all">
                    <div className="text-[10px] font-bold text-slate-500 uppercase mb-0.5 truncate">{w.groups.length > 3 ? 'All Groups' : w.groups.join(', ')}</div>
                    ...
                    <div className="absolute right-1 top-1 hidden group-hover:flex gap-1 bg-white/90 backdrop-blur-sm p-0.5 rounded shadow-sm border border-slate-100">
                         <button onClick={(e) => { e.stopPropagation(); onRepeatWorkout(w.workout); }} className="text-brand-600 hover:bg-brand-50 p-1 rounded"><Copy className="w-3 h-3" /></button>
                        <button onClick={(e) => { e.stopPropagation(); onDeleteWorkout(w.id); }} className="text-red-500 hover:bg-red-50 p-1 rounded"><Trash2 className="w-3 h-3" /></button>
                    </div>
                </div>
            ))}
        </div>
