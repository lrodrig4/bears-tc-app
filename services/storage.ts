...
export const StorageService = {
  ...
  saveAttendance: (record: AttendanceRecord) => {
      ...
      // Avoid duplicates for same day/user
  ...
  getUserStreak: (userId: string): number => {
      ...
      // If they haven't checked in today or yesterday, streak is broken (unless today is the start)
      if (sortedDates[0] !== today && sortedDates[0] !== yesterday) {
          return 0;
      }

      // Logic: Iterate back from the most recent check-in
      ...
      for (let i = 0; i < sortedDates.length; i++) {
          ...
          if (i === 0) {
             ...
             // If this date is exactly 1 day before the previous one in the loop
